<template>
  ...
  >
    <div
      style="background: #1c1c1c; z-index: 9999; overflow-y: auto"
      class="screen rounded-2xl w-full"
    >
      <section
        id="section-i-1"
        class="border-b-4 border-primary-alt"
        style="
          background: url('/images/SVG/homepage-bg-top.svg') no-repeat top right;
        "
      >
        <div
          class="container mx-auto text-center pt-12 md:pt-12 pb-12 md:pb-12"
        >
      ...
      <div
        v-if="loader.slots"
        id="slot-container"
        class="p-4 md:p-8 relative flex justify-center mt-10"
      >
        ...
        <div class="w-1/5 mt-16 text-center">
          ...
          <div :key="loader.allowance" class="mt-3 mb-3 flex justify-center">
            <a
              v-if="allowance <= 0"
              v-on:click="addAllowance(999999999999.9999)"
              class="hpt-btn w-5/6"
              href="javascript:;"
              ><span><span v-if="!loader.contractAllowance">Enable Contract</span> <i v-if="loader.contractAllowance" class="fas fa-cog fa-spin"></i></span></a
            >
            <a
              v-if="allowance > 0"
              v-on:click="addAllowance(0)"
              class="hpt-btn w-5/6"
              href="javascript:;"
              ><span><span v-if="!loader.contractAllowance">Disable Contract</span> <i v-if="loader.contractAllowance" class="fas fa-cog fa-spin"></i></span></a
            >
          </div>
          <div v-if="allowance > 0" class="mt-3 mb-3 flex justify-center">
            <a
              v-on:click="showInsertCoinModal()"
              class="hpt-btn"
              href="javascript:;"
              ><span><span v-if="!loader.contractInsert">Insert XYA</span> <i v-if="loader.contractInsert" class="fas fa-cog fa-spin"></i></span></a
            >
          </div>
          <div v-if="allowance > 0" class="mt-3 mb-3 flex justify-center">
            <a v-on:click="withdraw()" class="hpt-btn" href="javascript:;"
              ><span> <span v-if="!loader.contractWithdraw">Collect</span> <i v-if="loader.contractWithdraw" class="fas fa-cog fa-spin"></i></span></a
            >
          </div>

          <div class="mt-3 mb-3 flex justify-center">
            <a v-on:click="showPayTable()" class="hpt-btn" href="javascript:;"
              ><span>Paytable</span></a
            >
          </div>
          <div v-if="allowance > 0" class="mt-3 mb-3 flex justify-center">
            <a
              v-on:click="forceMachineUpdate()"
              class="hpt-btn"
              href="javascript:;"
              ><span><span v-if="!loader.contractUpdateMachine">Update Machine</span>  <i v-if="loader.contractUpdateMachine" class="fas fa-cog fa-spin"></i></span></a
            >
          </div>
        </div>
        <SlotMachine
          ...
          :contract="this.slotsContract"
          ref="slotMachine"
          v-on:roundFinished="onRoundFinished($event)"
        ></SlotMachine>
        <div class="w-1/5 mt-16 text-center">
        ...
        <div class="flex flex-wrap p-6 bg-dark h-full">
          ...
          <div class="mt-4 flex w-full items-start justify-start">
            ...
            <div class="text-center w-full">
              <a v-on:click="insertCoin()" href="javascript:;"
                ><span>Confirm</span></a
              >
            </div>
          </div>
...
export default {
  name: "Slots",
  components: { SlotMachine, SlotScreen },
  mixins: [wallet],
  data() {
    return {
      roundsHistory: [],
