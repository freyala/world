<template>
  <div class="flex flex-wrap h-16 absolute z-50 top-0 left-0 p-4 md:p-8 opacity-90">
    ...
    <div v-if="" class="w-auto -ml-14 z-10 md:h-12 pt-3">
    ...
    <div class="w-full mt-2 md:mt-0">
      <div class="flex md:ml-20">
        <div :key="favourite" v-for="favourite in favourites" class="md:opacity-75 hover:opacity-100 w-12 h-12 cursor-pointer flex fav-bg mr-2 bgshadow">
          <router-link class="m-auto max-h-9 max-w-9" :to="{ name: favourite === 'delegate' ? 'delegating' : favourite }">
            ...
            <img style="max-height: 50px;" class="p-1 -mt-1" v-if="favourite === 'plots'" src="/images/map/FreyalaPlots.png" alt="Plots">
          </router-link>
        </div>
      </div>
    </div>
