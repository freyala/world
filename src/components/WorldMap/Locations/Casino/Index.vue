    ...
    <div class="slide-in shadow-xl" v-if="openWindow === 'casino' || openWindow.startsWith('casino-large')"
      ...
      <div class="flex text-center mt-4">
        <div class="ml-auto w-60 pr-2">
          <button type="button" class="w-full rounded-none border border-yellow bg-transparent hover:bg-yellow hover:text-brown px-4 py-2 min-h-12" @click="openWindow === 'casino-large-roulette' ? $store.dispatch('setOpenWindow', 'casino') : $store.dispatch('setOpenWindow', 'casino-large-roulette')">
            {{ openWindow === 'casino-large-roulette' ? 'Leave' : 'roulette lobby' }}
          </button>
        </div>
        <div class="mr-auto w-60 pl-2">
          <button type="button" class="w-full rounded-none border border-yellow bg-transparent hover:bg-yellow hover:text-brown px-4 py-2 min-h-12" @click="openWindow === 'casino-large-coinflip' ? $store.dispatch('setOpenWindow', 'casino') : $store.dispatch('setOpenWindow', 'casino-large-coinflip')">
