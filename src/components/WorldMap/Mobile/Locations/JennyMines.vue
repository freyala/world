<template>
  <transition name="slide" v-if="chainStatus === 'correct' && walletConnected">
    <div class="slide-in shadow-xl w-full" v-if="openWindow === 'jennymines'"
      ...
      <div class="flex relative cursor-pointer mt-6" @click="setFavourite('jennymines')">
        <div class="text-center w-full" style="top: 80px; line-height: 0.25">
          <small>Click to {{ favourites.includes('jennymines') ? 'unfavourite' : 'favourite' }}</small>
          <p class="text-xl">Jenny mines</p>
        </div>
      </div>
