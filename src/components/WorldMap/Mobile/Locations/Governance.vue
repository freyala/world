<template>
  <transition name="slide" v-if="walletConnected && chainStatus === 'correct'">
    <div class="slide-in shadow-xl w-full" v-if="openWindow === 'governance'"
      ...
      <div v-if="governanceMounted">
        ...
        <div class="flex relative cursor-pointer mt-6" @click="setFavourite('governance')">
          <div class="text-center w-full" style="top: 80px; line-height: 0.25">
            <small>Click to {{ favourites.includes('governance') ? 'unfavourite' : 'favourite' }}</small>
            <p class="text-xl">Everglen Castle</p>
          </div>
        </div>
        ...
        <div class="flex flex-wrap mb-12 mt-8">
          <div class="w-full">
            <div class="text-center" v-html="governanceFetchedData.currentBallot.name"></div>
