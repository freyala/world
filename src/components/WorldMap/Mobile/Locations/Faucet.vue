<template>
  <transition name="slide" v-if="chainStatus === 'correct' && walletConnected && faucetMounted">
    <div class="slide-in shadow-xl w-full" v-if="openWindow === 'faucet'"
      ...
      <div class="flex relative cursor-pointer mt-6" @click="setFavourite('faucet')">
        <div class="text-center w-full" style="top: 80px; line-height: 0.25">
          <small>Click to {{ favourites.includes('faucet') ? 'unfavourite' : 'favourite' }}</small>
          <p class="text-xl">Soup kitchen</p>
        </div>
      </div>
      <br>
