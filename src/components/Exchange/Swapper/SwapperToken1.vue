    ...
    <div class="flex flex-col space-y-1 w-1/3 min-w-0">
      <div class="relative flex space-x-2 items-center h-10">
        <i @click="addTokenToMetamask(this.getToken()['token2'])"
           class="flex las la-plus text-sm p-1 text-yellow absolute top-0 left-0 items-center justify-center bg-slightGray dark:bg-slightDark rounded-full cursor-pointer shadow hover:bg-yellow dark:hover:bg-yellow hover:text-slightGray dark:hover:text-slightDark"></i>
        <img :src="this.getToken()['token2'].imgSrc" class="h-8 w-8 rounded-full flex items-center justify-center"
             alt="">
        <div class="flex flex-1 items-center min-w-0">
          <p class="text-sm text-el text-yellow-dark">{{ this.getToken()['token2'].Symbol }}</p>
        </div>
      </div>
      ...
      <div class="flex h-10">
        <Input1 :errors="errors" :rounded="'rounded-xl'" :placeholder="'Amount...'" :errorTop="'pt-10'">
          <div class="flex flex-1 st5-all items-center justify-end group-scope z-30 absolute right-0">
            <div @click="setMax()"
                 class="flex h-10 items-center bg-gray-100 dark:bg-Dark-gray group-scope-hover:bg-yellow text-yellow-dark dark:text-yellow border-l border-black border-opacity-10 cursor-pointer px-3 rounded-xl">
