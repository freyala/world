  ...
  <div class="flex flex-1 items-center">
    <div class="flex flex-col space-y-1 w-1/3 min-w-0">
      <div class="relative flex space-x-2 items-center h-10">
        <i @click="addTokenToMetamask(this.getToken()['token1'])" class="flex las la-plus text-sm p-1 text-primary-head absolute top-0 left-0 items-center justify-center bg-slightGray dark:bg-slightDark rounded-full cursor-pointer shadow hover:bg-primary-head dark:hover:bg-primary-head hover:text-slightGray dark:hover:text-slightDark"></i>
        <img :src="this.getToken()['token1'].imgSrc" class="h-8 w-8 rounded-full flex items-center justify-center" alt="">
        <div class="flex flex-1 items-center min-w-0">
          <p class="text-sm text-el text-primary-head-dark">{{this.getToken()['token1'].Symbol}}</p>
        </div>
      </div>
    ...
    <div class="flex flex-col space-y-1 flex-1 w-2/3 min-w-0">
      <div class="flex h-10">
        <Input0 :errors="errors" :rounded="'absolute bottom-0 left-4 text-white opacity-80 pb-2'" :placeholder="'Amount...'" :errorTop="'pt-10'">
          <div class="flex flex-1 st5-all items-center justify-end group-scope z-30 absolute right-0">
            <div @click="setMax()" class="flex h-10 items-center bg-gray-100 dark:bg-Dark-gray group-scope-hover:bg-primary-head text-primary-head-dark dark:text-primary-head border-l border-black border-opacity-10 cursor-pointer px-3 absolute bottom-0 left-4 text-white opacity-80 pb-2">
