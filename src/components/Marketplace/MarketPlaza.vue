<template>
  <div :key='keys.application' class='w-full h-full flex flex-row'>
    ...
    <div v-show='loaders.application'
      ...
      <h2 class='text-primary-head text-2xl'>Loading...</h2>
    </div>
    ...
    <div
        class='xl:relative absolute 2xl:w-1/5 xl:w-3/12 lg:w-5/12 sm:w-6/12 w-full h-full flex flex-wrap py-4 px-4 pt-16 overflow-y-auto overflow-x-hidden top-diff'
      ...
      <div
          class='text-center w-full flex flex-col justify-center items-center mt-4 pb-4 shadow-lg bg-detail-bg dark-panel'
          style='height: 67px;'>
        <div class='w-9/12 mx-auto flex h-full'>
          <button v-on:click='resetMarketFilters()' type="button"
                  class="w-9/12 mx-2 hover:text-primary-head px-0 py-0 min-h-9">
            Reset
          </button>
      ...
      <div :key='keys.filters'
           class="overflow-y-auto w-full text-center flex py-4 my-4 flex-col bg-detail-bg dark-panel"
        ...
        <div class='w-full mb-2'>
          ...
          <div class='flex w-9/12 mx-auto'>
            <select v-model='marketIdQueryComparator'
                    class="w-9/12 xya-input text-sm py-2 px-4 bg-detail-bg">
              <option value='tokenId'>Equal</option>
              <option value='tokenId_lte'>Lesser Than</option>
              <option value='tokenId_gte'>Greater Than</option>
            </select>
            <input v-model='marketIdQuery' v-bind:class='{"text-primary-head": marketIdQuery > 0}' type='number'
                   class='w-6/12 ml-4 xya-input py-2 px-4 bg-detail-bg'/>
          </div>
        </div>
        ...
        <div class='w-full mb-2'>
          ...
          <select v-model='marketSelectedCurrency'
                  class="w-9/12 xya-input py-2 px-4 bg-detail-bg">
        ...
        <hr v-if='!bools.onlySales' class='w-9/12 opacity-75 mx-auto my-6'>
        <template v-if='!bools.onlySales'>
          <div v-for='(attribute, index) in marketAttributes' :key='index' class="w-full mb-2">
            ...
            <select v-model='marketSelectedFilters[index]'
                    class="w-9/12 xya-input py-2 px-4 bg-detail-bg"
                    v-bind:class='{"text-primary-head": marketSelectedFilters[index] !== ""}'>
    ...
    <transition name='filters'>
      <div v-show='bools.responsiveFilters'
           class='h-auto top-0 left-0 xl:hidden absolute h-full md:w-8/12 w-full 2xl:text-base md:text-sm bg-light z-50'
        ...
        <div :key='keys.filters' class="text-center overflow-y-auto flex py-4 mx-2 flex-col bg-light"
          ...
          <div class='w-full mb-2'>
            <!--
            <div class='w-full mb-2'>
                <div v-on:click='bools.onlySales = !bools.onlySales' class='flex w-9/12 mx-auto'>
                    <div v-bind:class='{"checked": bools.onlySales}' class='checkbox cursor-pointer'>
                    </div>
                    <div class='mb-1 ml-4'>
                        Show Recent Sales
                    </div>
          ...
          <div class='w-full mb-2'>
            ...
            <select :key='marketSelectedCurrency' v-on:change='initiateMarketSearch()'
                    v-model='marketSortBy' class="w-9/12 xya-input py-2 px-4 bg-detail-bg">
              <option v-bind:value='""'>Order By</option>
    ...
    <div
        class='xl:w-10/12 w-full h-full min-h-100 flex flex-col overflow-x-hidden sm:overflow-y-hidden overflow-y-scroll mt-2'>

      <div
          class='w-full mx-auto flex md:justify-end justify-evenly md:flex-row flex-col py-4 select-none text-center shadow-xl'>
        ...
        <h2 v-on:click='marketTab = CONSTANTS.SALES_TAB'
            v-bind:class='{"opacity-25": marketTab !== CONSTANTS.SALES_TAB, "text-primary-head": marketTab === CONSTANTS.SALES_TAB}'
            class='2xl:text-xl z-0 lg:text-lg flex md:text-base text-xl mt-1 cursor-pointer hover:opacity-100 md:mb-0 mb-3 md:w-1/5 mx-auto w-full xya-btn relative'>
          <span class="m-auto text-white">Sales</span>
        </h2>
        <h2 v-on:click='marketTab = CONSTANTS.USER_TAB'
            v-bind:class='{"opacity-25": marketTab !== CONSTANTS.USER_TAB, "text-primary-head": marketTab === CONSTANTS.USER_TAB}'
            class='2xl:text-xl z-0 lg:text-lg flex md:text-base text-xl mt-1 cursor-pointer hover:opacity-100 md:mb-0 mb-3 md:w-1/5 mx-auto w-full xya-btn relative'>
          <span class="m-auto text-white">My Sales ({{ userSales.length }})</span>
        </h2>
        <h2 v-on:click='marketTab = CONSTANTS.COLLECTION_TAB'
            v-bind:class='{"opacity-25": marketTab !== CONSTANTS.COLLECTION_TAB, "text-primary-head": marketTab === CONSTANTS.COLLECTION_TAB}'
            class='2xl:text-xl z-0 lg:text-lg flex md:text-base text-xl mt-1 cursor-pointer hover:opacity-100 md:mb-0 md:w-1/5 mx-auto w-full mb-3 xya-btn relative'>
          <span class="m-auto text-white">My NFT's ({{ userTokens.length }})</span>
        </h2>
      </div>

      <div ref='container' class='h-5/6 w-full mx-auto py-4 sm:overflow-y-scroll'>
        <!-- SALES -->
        <div :key='keys.marketSales' v-show='marketTab === CONSTANTS.SALES_TAB'
          ...
          <div class='text-xl md:p-4 p-0 md:w-6/12 w-8/12 opacity-75' v-if='marketTokens.length === 0'>
            There are no sales.
          </div>
          <MarketPlazaItem :isBusy='item.isBusy' class='mt-2 2xl:mx-4 md:mx-2 mb-6 flex flex-col'
            ...
            <div v-on:click='showMarketCardModal(item)' class='market-item-header relative' slot='header'>
              <div
                  class='bg-detail-bg shadow-lg absolute text-primary-head text-base flex items-center justify-center top-3 left-0 w-6/12 h-6'>
              ...
              <div v-if='item.type === CONSTANTS.AUCTION && item.highestBidder'
                   v-bind:class='{"bg-detail-bg": metaMaskAccount.toLowerCase() !== item.highestBidder, "bg-green": metaMaskAccount.toLowerCase() === item.highestBidder}'
                   class='absolute shadow-lg  text-primary-head text-base flex items-center justify-center top-12 left-0 w-3/12 h-6'>
                                <span class='text-sm'>
            ...
            <div class='px-1 market-item-body' slot='body'>
              ...
              <div class='ml-2 w-full ml-2 mr-2 flex'>
                ...
                <div class='w-full text-right mr-4 text-primary-head text-primary-head'>
                  {{ (item.price / (10 ** 18)).toFixed(1) }}
                  {{ getCurrencyName(item.currency.id) }}
                </div>
              </div>
              <div v-if='false' class='w-full mt-2 ml-2 mr-2 flex text-primary-head opacity-50'>
                <div class='w-8/12 text-right mr-2 my-auto mt-1 text-xs'>Last</div>
                <div class='w-4/12'>10 XYA</div>
              </div>
            </div>
            <div slot='footer' class='market-item-footer p-2 mt-auto flex flex-wrap justify-evenly'>
              <button v-on:click='showMarketCardModal(item)' type="button" class="w-2/3 mx-auto xya-btn relative mb-4">
                Details
              </button>
              <button v-if='item.type === CONSTANTS.SALE' v-on:click='buyMarketNFT(item)' type="button"
                      class="w-2/3 mx-auto xya-btn relative mb-4">
                <span>Buy</span>
              </button>
              <button v-if='item.type === CONSTANTS.AUCTION && !item.ended'
                      v-on:click='showMakeBidModal(item)' type="button" class="w-2/3 mx-auto xya-btn relative mb-4">
                <span>Bid</span>
              </button>
            </div>
          </MarketPlazaItem>
          ...
          <MarketPlazaItem :isBusy='item.isBusy' class='mt-2 2xl:mx-4 md:mx-2 mb-6 flex flex-col'
            ...
            <div class='px-1 market-item-body' slot='body'>
              ...
              <div class='ml-2 w-full ml-2 mr-2 flex'>
                ...
                <div class='w-full text-right mr-4 text-primary-head'>
                  {{ (item.price / (10 ** 18)).toFixed(1) }}
                  {{ getCurrencyName(item.currency.id) }}
                </div>
              </div>
            ...
            <div slot='footer' class='relative market-item-footer p-2 mt-auto flex flex-wrap justify-evenly'>
              <button v-on:click='showCollectionSaleModal(item)' class="w-2/3 mx-auto xya-btn relative mb-4">
                Details
              </button>
              <button v-on:click='delistNFT(item)' class="w-2/3 mx-auto xya-btn relative mb-4">
          ...
          <MarketPlazaItem :isBusy='item.isBusy' class='mt-2 2xl:mx-4 md:mx-2 mb-6 flex flex-col'
            ...
            <div slot='footer' class='relative market-item-footer p-2 mt-auto flex flex-wrap justify-evenly'>
              ...
              <button id='collectionMenuButton' v-bind:data-token-id='item.tokenId'
                      v-on:click='showCollectionDropdown' type="button" class="w-2/3 mx-auto xya-btn relative mb-4">
    ...
    <div id='collectionMenu' style='min-width: 128px' v-show='bools.collectionMenu' ref='collectionMenu'
         class='fixed border shadow-xl px-6 py-2 text-center bg-detail-bg cursor-pointer'>
      <div v-if='isFreyMarket' v-on:click='showPickCurrencyModal()' class='my-4 hover:text-primary-head'>
        Register
      </div>
      <div v-on:click='showSendNFTModal()' class='my-4 hover:text-primary-head'>
        Send
      </div>
    </div>
          ...
          <div class='w-full md:h-3/6 h-2/6'>
            <div class='w-full flex items-center md:h-8 h-0'>
              ...
              <span v-on:click='showSendNFTModal()' title='Send' data-tooltip-target="tooltip-send"
                    data-tooltip-placement="left" class='cursor-pointer mx-2 ml-auto hover:text-primary-head'><i
                  class="fas fa-paper-plane"></i></span>
              <span v-if='isFreyMarket' v-on:click='showPickCurrencyModal()' title='Register'
                    class='cursor-pointer mx-2 hover:text-primary-head'><i class="fas fa-check"></i></span>
              <span v-on:click='bools.collectionCard = false'
                    class="fas mx-2 text-xl ml-4 fa-times cursor-pointer text-xl hover:text-primary-head">
            ...
            <h2 v-if='isFreyMarket' class='md:text-sm text-sm md:mt-2 md:mb-0 md:mt-0 mb-4 mt-4'><span
                class='text-primary-head opacity-50'>Frey Fees:</span>
              <strong class='text-primary-head'> {{ collectionSelectedNFTFees }}</strong></h2>
          </div>
          <div class='w-full flex flex-wrap md:flex-row md:my-0 md:mt-0 mt-2 pb-8 h-3/6'>
            <button v-on:click='showCreateMarketSaleModal()' type="button"
                    class="md:ml-auto w-2/3 mx-auto mb-4 xya-btn relative">
              Sale
            </button>
    ...
    <transition name="market-modal">
      ...
      <MarketModal v-if='bools.collectionSale && collectionSelectedNFT' v-on:closeModal='closeMySaleModal()'
                   :item='collectionSelectedNFT' :tokens='this.acceptedTokens'
          ...
          <div class='w-full md:h-2/6 h-auto'>
            <div class='w-full flex items-center md:h-8 h-0'>
              ...
              <span v-on:click='getShareURL()' title='Send'
                    class='cursor-pointer mx-2 ml-auto hover:text-primary-head'><i
            ...
            <h2 class='text-base text-primary-head opacity-50 mt-1 md:my-1'>Owned by: You</h2>
          </div>
          <div class='w-full flex md:flex-nowrap flex-wrap md:flex-col flex-row md:h-4/6'>
            ...
            <h2 v-else-if='collectionSelectedNFT.highestBidder || collectionSelectedNFT.ended'
              ...
              Highest Bidder: <span
                class='text-sm text-primary-head opacity-75'>{{
                getAccountStamp(collectionSelectedNFT.highestBidder)
              }}</span>
            </h2>
            ...
            <div class='flex flex-row md:flex-row w-full md:items-center mt-0 md:mt-0 items-center'>
              <h2 class='text-xl text-primary-head'>
              ...
              <button v-if='collectionSelectedNFT.type === CONSTANTS.SALE'
                      ...
                      class="xya-btn relative ml-auto mr-8 w-6/12 md:w-4/12 mt-2 md:mt-0">
                Cancel
              </button>
            </div>
            ...
            <div v-else-if='collectionSelectedNFT.type === CONSTANTS.AUCTION'
              ...
              <h2 class='text-lg md:text-xl mt-2 text-primary-head'>
                Auction Ended
              </h2>
      ...
      <MarketModal :tokens='this.acceptedTokens' v-if='bools.marketCard && marketSelectedNFT'
        ...
        <template slot='header-content'>
          <div class='w-full h-2/6'>
            ...
            <h2 class='text-base text-primary-head opacity-50 md:mb-1 mb-2'>Owned by:
              {{ getAccountStamp(marketSelectedNFT.seller.id) }}
            </h2>

          </div>
          <div class='w-full h-4/6 flex flex-col mt-auto'>
            ...
            <div class='flex w-full items-center'>
              ...
              <h2 v-if='marketSelectedNFT.highestBidder'
                  class='text-primary-head opacity-50 text-sm mx-3 hidden lg:block'>
                {{ getAccountStamp(marketSelectedNFT.highestBidder) }}
              </h2>
              <h2 v-if='marketSelectedNFT.highestBidder'
                  class='text-primary-head opacity-50 text-sm mx-3 block lg:hidden'>
                {{ getAccountStamp(marketSelectedNFT.highestBidder) }}
              </h2>
              <button v-if='marketSelectedNFT.type === CONSTANTS.SALE'
                      v-on:click='buyMarketNFT(marketSelectedNFT)' type="button"
                      class="xya-btn relative ml-auto w-4/12 mr-8">
                <span>Buy</span>
              </button>
              <button
    ...
    <window v-if='bools.collectionCard && collectionSelectedNFT' name="create-listing" width='80%' height='auto'>
      <div class="flex flex-wrap flex-col p-6 bg-detail-bg h-full">
        <div class='w-full flex'>
          <div class="w-4/5">
            <div v-if='collectionSaleType === CONSTANTS.SALE' class="text-2xl">List
              #{{ collectionSelectedNFT.tokenId }} for sale
            </div>
        ...
        <div class="mt-2 mb-4 flex w-full">
          <input class='w-full text-black px-2 h-9 shadow-xl border border-detail-bg'
                 type="number" v-model='collectionSaleAmount'/>

          <select v-model='collectionSaleToken'
                  class="w-6/12 xya-input ml-4 px-4 bg-detail-bg">
        ...
        <div v-if='collectionSaleType === CONSTANTS.AUCTION' class="mt-1 mb-4 flex h-12 w-full px-0 py-1">
          <select v-model='collectionSaleDuration'
                  class="w-full xya-input mr-2 py-2 px-2 bg-detail-bg text-sm md:text-base">
          ...
          <select v-on:change='getIntervalLimit(collectionSaleDurationType)'
                  ...
                  class="w-6/12 xya-input ml-2 py-2 px-2 bg-detail-bg text-xs md:text-base">
        ...
        <div class='w-full flex justify-space p-1 mt-4 text-base'>
          <div class='md:w-1/3'></div>
          <button v-on:click="$modal.hide('create-listing')"
                  class="w-1/3 md:w-1/4 mx-auto my-auto hover:border hover:border-detail-bg bg-transparent hover:text-primary-head md:text-xl text-base">
            Cancel
          </button>
          <button v-on:click="listNFT(collectionSelectedNFT, collectionSaleType)"
                  class="w-1/3 md:w-1/4 mx-auto my-auto xya-btn relative md:text-xl text-base">
            Confirm
          </button>
        </div>
      </div>
    ...
    <window class='w-10/12' height="auto" width="80%" name="pick-currency">
      <div class="flex flex-wrap p-6 bg-detail-bg h-full">
        ...
        <div class="mt-4 flex md:flex-row flex-col w-full items-center justify-start">
          <select v-model='collectionSaleToken'
                  class="w-full md:w-9/12 h-12 xya-input px-4 bg-detail-bg">
            <option v-for='(token, index) in acceptedTokens' v-bind:value="token.value" :key='index'>
              {{ token.key }}
            </option>
          </select>
          <div class="text-right md:text-center mt-4 md:mt-0 md:w-9/12 w-full">
            <button v-on:click="registerFreyNFT(collectionSelectedNFT)" type="button"
                    class="w-full md:mx-4 mt-auto xya-btn md:text-xl text-base">
              <span>Confirm</span>
            </button>
          </div>
        </div>
    ...
    <window class='w-10/12' height="auto" width="80%" name="market-stats">
      <div class="flex flex-wrap p-6 bg-detail-bg h-full">
        ...
        <div v-for='(freyReflectionData, index) in marketStatsReflections' :key='index'
          ...
          <div>
            <strong class='ml-4 text-primary-head'>{{ freyReflectionData.value }}</strong>
          </div>
        </div>
      </div>
    </window>
    ...
    <window class='w-10/12' height="auto" width="80%" name="send-NFT">
      <div class="flex flex-wrap p-6 bg-detail-bg h-full">
        ...
        <div class="mt-4 flex md:flex-row flex-row w-full items-start justify-start">
          <input class='w-full text-black px-2 absolute bottom-0 left-4 text-white opacity-80 pb-2' type="text"
                 v-model='NFTTransactionTo'/>
          <div class="text-right md:text-center md:w-9/12 w-5/12 mx-2 md:mx-0">
            <button v-on:click="sendNFT(collectionSelectedNFT)" type="button"
                    class="w-full md:w-10/12 md:text-base text-sm border border-detail-bg bg-transparent hover:bg-detail-bg hover:text-primary-head px-2 mx-2 py-0">
              <span>Confirm</span>
            </button>
          </div>
        </div>
    ...
    <window class='w-10/12' height="auto" width='80%' name='force-withdraw'>
      <div class="flex flex-wrap p-6 bg-detail-bg h-full">
        ...
        <div class="mt-4 flex md:flex-row flex-row w-full items-start justify-start">
          <input class='w-full xya-input p-4' type="text"
                 v-model='forceWithdrawTokenId'/>
          <div class="text-right md:text-center md:w-9/12 w-5/12 my-auto mx-2 md:mx-0">
        ...
        <div class="mt-4 flex md:flex-row flex-row w-full items-start justify-start">
          <input class='w-full xya-input p-4' type="text"
                 v-model='forceCancelSellTokenId'/>
          <div class="text-right md:text-center md:w-9/12 w-5/12 my-auto mx-2 md:mx-0">
    ...
    <window class='w-10/12' height="auto" width="80%" name="make-bid">
      <div class="flex flex-wrap p-6 bg-detail-bg h-full">
        ...
        <div class="mt-4 flex md:flex-row flex-row w-full items-start justify-start">
          <input class='w-full text-black px-2 h-9 absolute bottom-0 left-4 text-white opacity-80 pb-2' type="text"
                 v-model='marketBidAmount'/>
          <div class="text-right md:text-center md:w-9/12 w-5/12 mx-2 md:mx-0">
    ...
    <window class='w-10/12' height="auto" width="80%" name="allowances">
      <div class="flex flex-wrap p-6 bg-detail-bg h-full">
        <div class="w-4/5">
          ...
          <div class='flex w-full flex-col md:flex-row my-2 items-center'>
            ...
            <div class="w-6/12 text-right md:text-center">
              <button v-on:click="setMarketApproval(!marketApproved ? 999999999 : 0)" type="button"
                      class="w-5/6 xya-btn relative mr-8">
          ...
          <div class='flex w-full items-center flex-row my-2' v-for='(token, index) in acceptedTokens'
            ...
            <div v-if='index > 0' class="w-6/12 text-right md:text-center mx-2">
              <button v-on:click="setTokenAllowance(token, !token.approved ? 999999999 : 0)" type="button"
                      class="w-5/6 ml-auto xya-btn relative mr-8">
    ...
    <window height='auto' width='80%' name='market-tutorial'>
      <div id='tutorial' class="flex flex-wrap p-6 bg-detail-bg h-full">
        <div class="w-4/5">
          ...
          <template v-if="tutorial.page === 3">
            ...
            <p class='txt-tutorial md:text-base text-sm'>
              ...
              by opening the User Panel window. <span><i
                class="ml-4 fas fa-user text-xl hover:text-primary-head"></i></span>
            </p>
            <br>
          </template>
        </div>
    ...
    <window height='50%' width='80%' name='user-profile'>
      <div :key='keys.feeBalance'
           class="flex flex-wrap p-6 md:text-base text-xs bg-detail-bg sm:overflow-y-hidden overflow-y-scroll"
        ...
        <div class="mt-4 flex flex-col w-full items-start justify-start">
          ...
          <div class='flex w-full flex-row items-center my-1' v-for='(token, index) in acceptedTokens'
            ...
            <p class='w-4/12 text-primary-head'>{{ token.balance }}</p>
            <button v-on:click="withdraw(token)" type="button"
                    class="w-4/12 ml-auto xya-btn relative mr-6 text-xs md:px-0 px-2 py-2 my-1 md:text-base">
              Collect
            </button>
          </div>
        </div>
        ...
        <div v-if='isFreyMarket' class="mt-4 flex flex-col w-full items-start justify-start">
          ...
          <div class='flex w-full items-center flex-row my-2' v-for='(token, index) in acceptedTokens'
               :key='index'>
            <p class='w-4/12 md:mr-0 mr-2'>{{ token.key }}</p>
            <p v-if='token.NFTBalance' class='w-4/12 text-primary-head'>{{ token.NFTBalance.toFixed(6) }}</p>
        ...
        <div class="mt-4 w-full flex flex-wrap">
          ...
          <div class='flex cursor-pointer justify-evenly mx-4 ' v-for='(token, index) in marketPendingTokens'
            ...
            <div class='flex flex-col text-center'>
              <img loading='lazy' v-on:click='withdrawNFT(token.tokenId)'
                   class='absolute bottom-0 left-4 text-white opacity-80 pb-2' width='64px'
...
export default {
  ...
  data() {
    return {
      ...
      CONSTANTS: {
        ...
        XYA_TOKEN: "0x9b68bf4bf89c115c721105eaf6bd5164afcc51e4"
      },
  ...
  watch: {
    ...
    'bools.onlySales': async function () {
  ...
  methods: {
    ...
    async fetchMarketSales() {
      try {
        ...
        if (this.bools.onlySales) {
          result = await this.$http.post(this.CONSTANTS.GRAPH_API, {
    ...
    getCurrencyQuery() {
      if (this.marketSortBy.indexOf('ONE') > -1) {
        this.marketSelectedCurrency = this.CONSTANTS.ONE_TOKEN;
      } else if (this.marketSortBy.indexOf('XYA') > -1) {
        this.marketSelectedCurrency = this.CONSTANTS.XYA_TOKEN;
      }
