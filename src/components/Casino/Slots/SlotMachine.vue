  ...
  <div class="pt-3 flex items-center justify-center relative select-none">
    ...
    <div class="machine-container flex items-center flex-col justify-center">
      ...
      <div class="screen medium inner-shadow flex flex-row justify-evenly">
        <transition name="fade">
          ...
          >
            ...
            <div>
              <img
                class="w-8 h-8 m-auto opacity-85"
                src="/images/XYA.png"
                alt="XYA logo"
        ...
        >
          <SlotReel
            v-for="i in reelCount"
            :reelBlockSize="96 - 10 * (reelCount - 3)"
            :key="i"
          ></SlotReel>
        </div>
        <transition name="nudge">
          <div
            v-if="showNudges"
            class="z-50 absolute flex justify-around bottom-5 w-80 h-3"
          >
            <div
              v-on:click="nudgeReel(i - 1)"
              class="nudge-button"
              v-for="i in 3"
              :key="i"
            >
      ...
      <div class="flex justify-evenly" style="width: 75%">
        <div v-for="i in 3" :key="i" class="w-4/12 h-16 p-4">
          <SlotButton
            v-on:buttonClick="holdGameReel(i - 1, $event)"
            :title="'Hold'"
            :info="'25'"
          >
          </SlotButton>
      ...
      <div style="height: 35%; width: 80%">
        ...
        >
          <template v-slot:content>
            <div
              v-show="!gameLoaded"
              class="w-full h-4/6 flex justify-center items-center text-center"
            >
              Fetching data...
            </div>
    ...
    <window name="payTable">
      <div class="flex flex-wrap p-6 bg-dark h-full">
        ...
        <div class="w-1/5 text-right">
          <i
            @click="$modal.hide('payTable')"
            class="fas fa-times cursor-pointer text-xl"
          ></i>
        </div>
        <div class="w-full">
          <div
            v-for="(payLine, i) in gamePayTable"
            :key="i"
            class="flex flex-row justify-evenly items-center m-2 mb-5"
          >
    ...
    <window name="error">
      <div class="flex flex-wrap p-6 bg-dark h-full">
        ...
        <p class="w-full mt-2 opacity-60">
          {{ gameError[1] }}
        </p>
      </div>
    </window>
...
export default {
  ...
  async mounted() {
    this.gameReels = this.$children.filter(
      (c) => c.$options.name === "SlotReel"
    );
    this.gameButtons = this.$children.filter(
      (c) => c.$options.name === "SlotButton"
    );
    this.startGameLoop();
    this.$nextTick(async () => {
  ...
  methods: {
    ...
    async fetchGameReels() {
      try {
        ...
        this.showGameError({message: "Transaction failed."});
      }
    },
    async startRound() {
      if (this.spamProtection > 0) return;
    ...
    async holdGameReel(index, validateClick) {
      try {
        ...
        if (helds.filter((c) => c == true).length >= 2 || !this.isLegalHold)
          throw {message: "You can't hold more than 2 reels at a time!"};

        if (holds == 0 || !holds) {
          throw {message: "You have 0 holds left."};
        }
    ...
    async nudgeReel(index) {
      try {
        if (!this.roundFinished) throw {message: "Invalid operation."};;
        const nudges = await this.contract.availableNudges();

        if (nudges === 0 || !nudges) {
          throw {message: "You have 0 nudges left."};
        }
