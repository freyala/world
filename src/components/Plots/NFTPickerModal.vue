<template>
  <div v-on:click='closeModal'
       class='fixed modal-overlay flex items-center justify-center top-0 bottom-0 right-0 left-0 bg-opacity-50 bg-detail-bg w-full h-full'
       style='z-index: 9999'>
    <div v-if='!showCollection'
         class='xl:w-6/12 w-10/12 sm:h-auto h-72 py-6 flex flex-col justify-start items-center bg-detail-bg dark-panel'>
      <h2 class='text-primary-head lg:text-4xl sm:text-3xl text-xl opacity-80 text-center mt-1'>Plot Slot {{
          slotNumber
        }}</h2>
      ...
      <div class='w-full h-3/5 sm:px-6 px-2 flex mb-6'>
        <div v-on:click='openCollectionTab(collection)'
             class='w-4/12 h-full lg:mx-6 mx-2 sm:pb-0 pb-2 dark-panel flex flex-col cursor-pointer justify-center'
          ...
          <img class='w-full h-3/5' v-lazy='collection.image'/>
          <div class='sm:h-12 h-10 mt-6 text-center w-full text-primary-head opacity-80 lg:text-2xl sm:text-xl text-xs'>
            {{ collection.name }}
          </div>
    ...
    <div v-else
         class='xl:w-6/12 w-10/12 h-auto py-2 pb-6 flex flex-col justify-start items-center bg-detail-bg dark-panel'>
      <h2 class='text-primary-head lg:text-4xl sm:text-3xl text-xl opacity-80 text-center mt-6'>Plot Slot {{
          slotNumber
        }}</h2>
      ...
      <div class="sm:w-8/10 w-9/10 p-4 mb-6 dark-panel">
        <select class="p-1 cursor-pointer xya-input sm:text-lg text-sm" style='width: 100%' name="neighbourhood"
                v-model="selectedNFT">
          <option v-for="(nft, index) in currentCollection.userNFTs" :value="index" :key='index'>
            {{ currentCollection.name }} - {{ resolveTokenId(nft) }}
          </option>
        </select>
      </div>
      <div
          class='sm:w-8/10 w-9/10 h-72 flex flex-row justify-center items-center overflow-y-hidden overflow-x-hidden dark-panel relative mb-2'>
        <div
            class='h-auto w-full flex sm:flex-row flex-col justify-center items-center sm:overflow-y-hidden overflow-y-scroll'>
          <img v-if='currentNFT' class='lg:4-/12 sm:w-5/12 w-6/12 mr-4 h-5/5 my-4' v-lazy='currentNFT.image'/>
          <div class='w-7/12 h-4/5 sm:items-start items-center flex flex-col'>
            ...
            <p class='lg:text-xl text-base mt-2 text-primary-head opacity-80'>Bonus</p>
            <p v-for='(bonus, index) in currentBonuses' :key='index' class='lg:text-xl sm:text-base text-xs mt-2'>
              {{ bonus }}</p>
          </div>
        </div>
      </div>
      <div v-if='approvedCollection' class='w-8/10 lg:mt-10 sm:mt-6 mt-2 h-auto flex flex-wrap flex-row justify-end items-center'>
        <button v-on:click='$emit("close")'
           class='sm:w-3/10 w-2/3 mx-auto lg:text-xl sm:text-lg text-base opacity-80 hover:text-primary-head relative text-center mx-4'>
          Cancel
        </button>
        <button v-on:click='placeNFT(currentCollection.address, currentNFT, slotNumber)'
           class='sm:w-3/10 w-2/3 mx-auto mt-5 sm:mt-0 lg:text-xl sm:text-lg text-sm xya-btn relative text-center'>
          Confirm
        </button>
      </div>
      <div class='w-8/10 my-4 h-auto flex flex-col justify-center items-center' v-else>
        <p class='text-primary-head text-start my-2 w-full opacity-60'>
          In order to stake NFT's, the staking contract must be approved for the current NFT collection!
        </p>
        <br>
        <button v-on:click='approveCurrentCollection(currentCollection)' class='w-4/10 xya-btn relative'>
          Approve Collection
        </button>
      </div>
    </div>
